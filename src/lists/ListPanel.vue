<!-- Sidebar launcher for a document's lists: one checkable row per list
  that toggles its canvas monitor. Data comes from the `lists` prop; open
  state lives in `./listEditors` (persisted via the host's configured save).
...
const props = defineProps<{ lists: readonly ListDef[] }>();
...
const sorted = computed(() => [...props.lists]
...
<template>
  <div class="list-launcher">
    <p v-if="!sorted.length" class="list-launcher-empty">Make a list, then check it to open its editor on the canvas.</p>
    <div v-for="list in sorted" :key="list.name" class="list-launcher-row" @contextmenu="emit('menu', list.name, $event)">
      <button
        type="button"
