...
// top-level instructions, or the nested slot of a wrap block. Extracted from
// StrandCard.vue so a wrap block's own field component can render its own
// nested body the same way: InstructionList -> InstructionRow -> (for a wrap
// row) -> that type's field component -> another InstructionList, and so on
// for arbitrary nesting depth.
//
// The root carries `.instruction-list` plus a serialized `data-path` (this
// list's own `basePath` — the steps needed to reach it from the strand's top
// level) so canvasDrag.ts's updateSnapTarget can find every drop target on
// the canvas and scope its boundary scan to each one's own *direct* child
// rows, without nested bodies' rows leaking into an ancestor's boundaries.
...
// (and is grabbable, to reposition the empty strand) — an empty nested body
